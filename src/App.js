...
function App() {
  ...
  const fetchTasks = async (token) => {
    const response = await fetch("https://todolist-zzty.onrender.com/tasks", {
  ...
  const deleteTask = async (id) => {
    await fetch(`https://todolist-zzty.onrender.com/tasks/${id}`, {
      method: "DELETE",
      headers: { Authorization: `Bearer ${token}` },
    });
  ...
  const updateTaskPriority = async (id, newPriority) => {
    const response = await fetch(
      `https://todolist-zzty.onrender.com/tasks/${id}/priority`,
      {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
